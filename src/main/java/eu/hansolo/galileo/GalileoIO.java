...
import static eu.hansolo.galileo.Pin.Direction;
import static eu.hansolo.galileo.Pin.Drive.STRONG;
import static eu.hansolo.galileo.Pin.Value;
...
public class GalileoIO {
    ...
    private void initPin(final Pin PIN, final Direction DIRECTION) {
        if (pinMap.keySet().contains(PIN) && pinMap.get(PIN) == DIRECTION) {
            ...
            if (PIN instanceof Digital) {
                ...
                *  "pullup"   : default but not suitable for driving devices that source significant current
                *  "pulldown" : Resistive low, strong high
                *  "strong"   : Strong low and high
                *  "hiz"      : CY98C9520A also supports open drain and open source drive modes
                */
                try (FileOutputStream out = new FileOutputStream("/sys/class/gpio/gpio" + ((Digital) PIN).gpio + "/drive")) {
                    PrintWriter pw = new PrintWriter(out);
                    pw.print(STRONG);
                    pw.close();
                } catch (IOException exception) {
                ...
                *  "hiz"      : CY98C9520A also supports open drain and open source drive modes
                */
                try (FileOutputStream out = new FileOutputStream("/sys/class/gpio/gpio" + ((Analog) PIN).gpio + "/drive")) {
                    PrintWriter pw = new PrintWriter(out);
                    pw.print(STRONG);
                    pw.close();
                } catch (IOException exception) {
                ...
                *  "hiz"      : CY98C9520A also supports open drain and open source drive modes
                */
                try (FileOutputStream out = new FileOutputStream("/sys/class/gpio/gpio" + ((GalileoPin) PIN).gpio + "/drive")) {
                    PrintWriter pw = new PrintWriter(out);
                    pw.print(STRONG);
                    pw.close();
                } catch (IOException exception) {
    ...
    public void setDigital(final Digital PIN, final Value VALUE) {
        ...
        try (FileOutputStream out = new FileOutputStream("/sys/class/gpio/gpio" + PIN.gpio + "/value")) {            
            switch(PIN) {
                case RX:
                    setGalileoPin(GalileoPin.GPIO_40, Value.LOW);
                    break;
                case TX:
                    setGalileoPin(GalileoPin.GPIO_41, Value.LOW);
                    break;
                case D0:
                    setGalileoPin(GalileoPin.GPIO_40, Value.HIGH);
                    break;
                case D1:
                    setGalileoPin(GalileoPin.GPIO_41, Value.HIGH);
                    break;
                case D2:
                    setGalileoPin(GalileoPin.GPIO_31, Value.HIGH);
                    break;
                case D3:
                ...
                case D9:
                    ...
                    break;
                case D10:
                    setGalileoPin(GalileoPin.GPIO_42, Value.HIGH);
                    break;
                case D11:
                    setGalileoPin(GalileoPin.GPIO_43, Value.HIGH);
                    break;
                case D12:
                    setGalileoPin(GalileoPin.GPIO_54, Value.HIGH);
                    break;
                case D13:
                    setGalileoPin(GalileoPin.GPIO_55, Value.HIGH);
                    break;
            }
            PrintWriter pw = new PrintWriter(out);
            pw.print(Value.HIGH == VALUE ? 1 : 0);
    ...
    public double getAnalog(final Analog PIN) {        
        ...
        switch(PIN) {
            case A0:
                setGalileoPin(GalileoPin.GPIO_37, Value.LOW);
            ...
            case A1:
                ...
                break;
            case A2:
                setGalileoPin(GalileoPin.GPIO_23, Value.LOW);
            ...
            case A3:
                ...
                break;
            case A4:
                setGalileoPin(GalileoPin.GPIO_21, Value.LOW);
                setGalileoPin(GalileoPin.GPIO_29, Value.HIGH);
